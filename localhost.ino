...
#define MAX_WIFI_RETRIES 1000  // 👈 you can set 3 or 5
...
void performFactoryReset() {
  ...
  // ===== 1. CLEAR SPIFFS =====
  Serial.println("🗑️ Formatting SPIFFS...");
  SPIFFS.format();
  delay(1000);

  // ===== 2. CLEAR WIFI =====
  Serial.println("📡 Clearing WiFi credentials...");
  WiFi.disconnect(true);   // erase stored SSID/password
  delay(500);

  WiFi.mode(WIFI_OFF);     // fully turn off WiFi
  delay(1000);

  // ===== 3. RESET WIFI MANAGER =====
  Serial.println("🔁 Resetting WiFiManager...");
  wm.resetSettings();
  delay(500);

  // ===== 4. 🔥 MOST IMPORTANT FIX =====
  Serial.println("🔥 Erasing SDK WiFi config...");
  ESP.eraseConfig();   // ✅ THIS FIXES YOUR ISSUE
  delay(1000);

  // ===== 5. FINAL RESTART =====
  Serial.println("🔄 Restarting clean device...");
  delay(2000);
...
WiFiClient client;
  ...
  if (code > 0) {
    ...
    if (deserializeJson(resp, res) == DeserializationError::Ok && resp["success"]) {
      ...
      if (token != readFromFile("/token.txt")) {
...
if (!nfc.tagPresent()) {
  ...
  // ===== TAG REMOVED =====performFactoryReset
  if (!tagNowPresent && cardWasPresent) {
    cardWasPresent = false;
...
if (millis() - lastCleanup > 300000) { // every 5 min
  Serial.println("🧹 Memory cleanup cycle");
  ESP.getFreeHeap();
  yield();
  lastCleanup = millis();
}
